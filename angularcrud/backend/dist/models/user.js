...
const create = (user, callback) => {
    const queryString = "INSERT INTO jsusers (nume, prenume, email, datanastere, adresa, telefon, cnp, poza) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    console.log(user);
    db_1.db.query(queryString, [user.nume, user.prenume, user.email, user.datanastere, user.adresa, user.telefon, user.cnp, user.poza], (err, result) => {
        if (err) {
            callback(err);
        }
