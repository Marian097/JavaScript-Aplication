...
export const create = (user: User, callback: Function) => {
  const queryString =
    "INSERT INTO jsusers (nume, prenume, email, datanastere, adresa, telefon, cnp, poza) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    console.log(user);
  db.query(
    queryString,
    [user.nume, user.prenume, user.email, user.datanastere, user.adresa, user.telefon, user.cnp, user.poza],
    (err, result) => {
      if (err) {
        callback(err);
      }
...
export const update = (user: User, callback: Function) => {
  ...
  db.query(queryString, [user.nume, user.prenume, user.telefon, user.cnp, user.poza, user.adresa, user.id], (err, result) => {
    if (err) {
      callback(err);
    }
    callback(null);
